...
- 只有在某个问题确实存在 2 个及以上合法选项时，才向用户输入工具传 `options`；若只有 1 个建议答案、只是给用户提示、或问题本质上需要自由输入，则省略 `options`，改用自由文本或把建议写进 `message`。
...
- 若使用用户输入工具且提供 `options`，则该问题必须至少有 2 个合法选项；不得构造只有 1 个选项的选择题。
